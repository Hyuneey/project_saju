# Manse Engine v0.4.1 Project Brief

## Goal

Manse Engine v0.4.1 calculates Korean saju four pillars from solar or Korean lunar birth-date input with deterministic, testable code. It is not an interpretation or fortune-telling layer.

v0.4.1 hardens the structured original-chart derived data added in v0.4.0. The calculation policy remains `manse-policy-v0.1`; the pillar formulas and solar-term dataset are unchanged from v0.2.3.
...
- Explicit derived `policyVersion` and `dataVersion`.
...
The derived original-chart layer is data-only. It consumes the calculated pillars and emits `policyVersion: derived-original-chart-v0.4.1` and `dataVersion: original-chart-derived-v0.4.1`.
